...
def chat(message):
    ...
    try:
        response = openai.ChatCompletion.create(model=history.model.name, messages=history.list)
        ...
        logger.error(error)
        try:
            bot.reply_to(message, text=print_error(error.args[0]), parse_mode="HTML")
            cache.set(message.chat.id, history)
        except Exception as error:
            logger.error(error)
